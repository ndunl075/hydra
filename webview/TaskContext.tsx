...
import { briefFields, buildTaskPrompt, canEditBrief, emptyBrief, emptyHandoffSummary, handoffFields } from '../src/core/taskContext';
...
export function PromptPreview({ prompt }: { prompt: string }) {
  return <details className="context-details"><summary>Exact initial prompt <span>{prompt.length.toLocaleString()} characters</span></summary><pre className="context-preview">{prompt || 'Add a goal to preview the prompt.'}</pre><p className="form-note">This is the text Hydra submits or copies. The provider may add its own workspace instructions and native session context.</p></details>;
}
...
export function TaskContext({ task, session, busy, send }: { task: Task; session?: SessionView; busy: boolean; send: (message: ClientMessage) => void }) {
  ...
  const editable = canEditBrief(task, session), prompt = buildTaskPrompt(brief);
  ...
  return <section className="task-context-panel" aria-label="Task brief and handoff">
    <details className="context-details"><summary>Task brief <span>{editable ? 'Editable before launch' : 'Initial context locked'}</span></summary>{editable ? <form onSubmit={event => { event.preventDefault(); send({ type: 'saveBrief', id: task.id, brief }); }}><BriefFields brief={brief} onChange={setBrief} disabled={busy} /><PromptPreview prompt={prompt} /><button className="secondary" disabled={busy || !brief.goal.trim() || prompt.length > 32000 || JSON.stringify(brief) === JSON.stringify(savedBrief)} type="submit">Save brief</button>{prompt.length > 32000 && <p role="alert">Shorten the brief to 32,000 characters.</p>}</form> : <><pre className="context-preview">{task.prompt}</pre><p className="form-note">The initial prompt stays fixed after launch. Send changes through an explicit follow-up.</p></>}</details>
