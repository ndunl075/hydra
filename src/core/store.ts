...
export class LocalStore {
  ...
  async load(): Promise<Task[]> {
    ...
    for (const value of data.tasks) {
      ...
      } else if (task.verificationEvidence !== undefined || task.delegationJournalPending !== undefined || task.delegationRetry !== undefined) throw new Error('Only delegated child tasks can retain delegation evidence. Original data has been retained.');
