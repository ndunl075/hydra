...
Hydra is its own Windows IDE, as clarified by Nico on 17 September 2026. Its workflow module is built into a branded Code - OSS desktop build; users do not need to install VS Code. A `.vsix` remains a core development artifact. Standalone acceptance passed in PR #11, and replayable onboarding merged in PR #18 after Linux and native checks. Settings import, provider account setup and Windows installer implementation are present in the combined stack. Account and final combined installer gates remain pending; see the [acceptance record](Implementation_Status.md#acceptance-record), [standalone build](Standalone_Build.md), [settings import](Settings_Import.md), [onboarding](Onboarding.md), [provider accounts](Provider_Account_Setup.md) and [Windows installer](Windows_Installer.md).
...
- Disposable Windows CI tests cover default-off, opt-in, remembered choice, explicit opt-out, uninstall cleanup, and preservation of Hydra preferences/extensions, VS Code/Cursor preferences and unrelated projects. These are same-version reinstall tests, with final combined lifecycle acceptance still pending.
...
4. Provider-owned account setup, pinned-runtime guidance, public status and cancellation are implemented in PR #24; its exact-revision native gate and live-provider acceptance remain separate pending checks. Login submits no model turn.
5. Installer generation and disposable lifecycle tests are implemented in PR #19. Accept the final combined revision's native build, smoke and installer lifecycle before publishing an installer; retain separate gates for distinct-version upgrades, visible wizard/launch, signing, distribution and updates.
