  ...
  show(): void {
    ...
    panel.webview.onDidReceiveMessage(async (message: unknown) => {
      try {
        ...
        if (await handleConnectionsMessage(action as Record<string, unknown>, value => panel.webview.postMessage(value))) return;
    ...
    return `<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'nonce-${nonce}'; script-src 'nonce-${nonce}';"><title>Hydra Settings</title>
      <style nonce="${nonce}">
        * { box-sizing: border-box; } body { margin: 0; color: var(--vscode-foreground); background: var(--vscode-editor-background); font: var(--vscode-font-size) var(--vscode-font-family); }
        ...
        .cards { display: grid; gap: 12px; margin: 18px 0; } .card { border: 1px solid var(--vscode-panel-border); border-radius: 6px; padding: 14px 16px; } .card .actions { margin: 10px 0 0; } .connection-state { margin: 0; color: var(--vscode-foreground); } .quiet { background: transparent; border-color: transparent; }
      ...
      <section aria-labelledby="helpers-heading"><h2 id="helpers-heading">Hydra helpers</h2><p>Connect Claude Code and Codex so their chats can start Hydra helpers: separate agents that work on independent pieces in their own worktrees.</p>${connectionsSection({ claude: '', codex: '' })}</section>
      ...
      <script nonce="${nonce}">
        const vscode = acquireVsCodeApi(); const status = document.getElementById('status'); const choices = [...document.querySelectorAll('[data-mode]')]; function send(message) { status.textContent = 'Working…'; vscode.postMessage(message); }
        ...
        window.addEventListener('message', event => { const message = event.data; if (message?.type === 'appearance') { choices.forEach(button => {button.disabled = false; button.setAttribute('aria-pressed', String(button.dataset.mode === message.mode));}); status.textContent = message.status || ''; } if (message?.type === 'connections') { renderConnections(message.connections); status.textContent = message.text || ''; } if (message?.type === 'error') { choices.forEach(button => button.disabled = false); status.textContent = message.text; } });
        vscode.postMessage({type:'ready'}); vscode.postMessage({type:'connections'});
