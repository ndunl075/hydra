...
function ProviderCheck({ provider, diagnostic }: { provider: Provider; diagnostic?: ProviderDiagnostic }) {
  return <section className="provider-check" aria-label={`${providerName(provider)} capabilities`}>
    ...
    <p className="quiet">Checks read public version and help output only. They make no model request or authentication check. Managed Claude supports version 2.1.270; interactive approvals and Codex streaming are unavailable.</p>
  </section>;
}
function SessionThread({ task, session, busy }: { task: Task; session: SessionView; busy: boolean }) {
  ...
  const blocked = busy || running || task.state === 'external' || task.interface === 'official-extension';
  return <>
    ...
    {session.turns.slice(-10).map(turn => <React.Fragment key={turn.id}>
      ...
      <article className="message assistant-message"><div className="message-author"><span className="avatar hydra-avatar">c</span><strong>Claude Code</strong><span className="local-tag">{turn.status === 'completed' ? 'TURN FINISHED' : turn.status.toUpperCase()}</span></div><p className="prompt-text">{turn.text || (turn.status === 'running' ? 'Waiting for provider output…' : 'No response text returned.')}</p>
    ...
    <form className="follow-up" onSubmit={event => { event.preventDefault(); if (prompt.trim()) { send({ type: 'followUp', id: task.id, prompt }); setPrompt(''); } }}>
      <label>Follow-up<textarea rows={3} maxLength={32000} value={prompt} disabled={blocked || !task.sessionId} onChange={event => setPrompt(event.target.value)} placeholder="Continue this task in its existing Claude session." /></label>
      ...
      <p className="form-note">Sending submits a model request. Follow-ups resume the recorded session after its process has ended. Stop process terminates the process tree; it does not finish the turn. Provider rules and hooks still apply; unresolved permissions are denied.</p>
    </form>
  </>;
}
    ...
    <div className="workspace">
      ...
      <section className="conversation" aria-label={creating ? 'New task' : 'Selected task'}>
        ...
        {snapshot.handoff ? <HandoffView handoff={snapshot.handoff} info={snapshot.officialExtensions?.find(info => info.provider === snapshot.handoff?.task.provider)} busy={snapshot.busy} /> : creating || !selected ? <>
          ...
          <div className="thread">
            {snapshot.session?.turns.length && selected.interface !== 'official-extension' && selected.provider === 'claude' ? <SessionThread key={selected.id} task={selected} session={snapshot.session} busy={snapshot.busy} /> : <>
            ...
            <article className="system-message"><div className="message-author"><span className="avatar hydra-avatar">h</span><strong>Hydra</strong><span className="local-tag">LOCAL</span></div><p>{selected.interface === 'official-extension' ? `This task is handed off to ${providerName(selected.provider)} in its own workspace window. Stop the provider session there before returning ownership.` : `Task checkout is ready. Open ${providerName(selected.provider)} in the terminal, then paste your prompt to begin.`}</p><div className="task-actions">{selected.interface === 'official-extension' ? <button className="secondary" disabled={snapshot.busy} onClick={() => send({ type: 'releaseExternal', id: selected.id })}>I stopped the external session</button> : <button className="primary" disabled={snapshot.busy || !provider?.available} onClick={() => send({ type: 'launch', id: selected.id })}><Icon name="terminal" />{selected.state === 'external' ? 'Show terminal' : 'Open provider terminal'}</button>}<button className="secondary" onClick={() => send({ type: 'copyPrompt', id: selected.id })}>Copy prompt</button></div>
              ...
              {selected.provider === 'claude' && selected.interface !== 'official-extension' && <><button className="secondary" disabled={snapshot.busy || selected.state === 'external' || !provider?.available} onClick={() => send({ type: 'startManaged', id: selected.id })}>Start managed Claude <Icon name="arrow" /></button><p className="observability">Submits the task to Claude CLI 2.1.270. Streams text and usage; unresolved permissions are denied. No interactive approval controls.</p></>}
