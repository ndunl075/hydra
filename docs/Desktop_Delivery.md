...
- Distinct-version upgrades, the visible wizard and launch-after-install require separate acceptance. The artifact is unsigned. It is distributed as a GitHub pre-release: [v0.23.0](https://github.com/ndunl075/hydra/releases/tag/v0.23.0) (2026-09-25) is the first, an unsigned Windows preview published from the tested CI installer at its merge commit. Code signing and automatic updates are not configured; users download new versions by hand. Installer tests must not run against a developer's personal installation.
