...
VerificationResult verify_signed_installer(HANDLE file, const wchar_t* absolute_path,
  ...
  const bool matched = status == 0 && data.hWVTStateData && signer_matches(data.hWVTStateData, allowed_signer, mismatch);
  ...
  if (!matched) return { false, status, mismatch.empty() ? L"verified signer is unavailable" : mismatch };
