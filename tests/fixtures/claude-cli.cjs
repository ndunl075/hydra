  ...
  process.stdin.on('data', text => {
    ...
    while (buffer.includes('\n')) {
      ...
      if (message.type === 'control_request') {
        ...
        const response = subtype === 'initialize' ? { current_permission_mode: scenario.permissionMode || 'default', account: { email: 'PRIVATE_ACCOUNT_MARKER' }, models: scenario.models || [{ value: 'claude-fixture-model', resolvedModel: 'claude-fixture-model', displayName: 'Claude fixture', supportsEffort: true, supportedEffortLevels: ['low', 'medium', 'high', 'xhigh', 'max'] }, { value: 'opus[1m]', resolvedModel: 'claude-fixture-model', displayName: 'Opus alias', supportsEffort: true, supportedEffortLevels: ['high', 'max'] }] } : subtype === 'get_settings' ? { applied, effective: { apiKeyHelper: 'PRIVATE_SETTINGS_MARKER' }, sources: [] } : subtype === 'get_binary_version' ? { version: scenario.version || '2.1.270' } : undefined;
