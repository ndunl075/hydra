...
test('the bridge picks the right window by folder, ignores dead windows, and explains when none is open', async () => {
  ...
  try {
    ...
    assert.deepEqual(listed.result.tools.map(tool => tool.name), leadTools.map(tool => tool.name).filter(name => name !== 'hydra_job_ready' && name !== 'hydra_active_roles'), 'hydra_job_ready is only for a plan lane; the roles lookup is only for the bridge itself');
