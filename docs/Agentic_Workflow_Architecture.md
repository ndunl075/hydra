...
### Workflow review decision (2026-09-18)

Adopt the following product patterns. This is a design decision, not runtime acceptance or an efficiency benchmark. These additions are planned, not shipped; retain Hydra's Code OSS foundation, Cursor-style Editor mode, and adaptive Solo/Auto policy.
...
- **Required checks block completion:** missing runners, absent evidence, interrupted checks, failed checks, and stale snapshot receipts cannot satisfy a required gate. Show the blocker and preserve work. A provider's completion message is not verification; combined integration acceptance still follows child checks. A missing runner must never be recorded as a pass, even with an "unverified" note.
...
Keep focused briefs, explicit missing-context requests, bounded retries, and deterministic dispatch from Hydra's existing design. Do not switch to an always-running coordinator or assume more agents save tokens.
