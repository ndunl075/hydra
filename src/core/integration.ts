...
export class Integrations {
  ...
  constructor(private readonly directory: string, private readonly changed: (op: IntegrationOperation)=>void = ()=>{}) { this.store=new IntegrationStore(path.join(directory,'operations')); }
  ...
  async prepare(task:Task,commands:IntegrationCommand[],guard:IntegrationGuard,signal?:AbortSignal):Promise<IntegrationOperation>{
    ...
    return this.locked(task,async()=>{
      await guard([task.worktree,task.repository]);const state=await inputs(task);
      ...
      try{
        ...
        await guard([task.worktree,task.repository]);await inputs(task,op);
  ...
  private async check(task:Task,op:IntegrationOperation,guard:IntegrationGuard,signal?:AbortSignal):Promise<void>{
    ...
    for(const check of op.checks){
      await guard([task.worktree,task.repository,op.candidate]);await inputs(task,op);await candidateIdentity(task,op);
  ...
  async reviewResolution(task:Task,op:IntegrationOperation,guard:IntegrationGuard):Promise<void>{
    await this.locked(task,async()=>{
      ...
      await guard([task.worktree,task.repository,op.candidate]);await inputs(task,op);await candidateIdentity(task,op);await recordCandidate(op);
  ...
  async promote(task:Task,op:IntegrationOperation,guard:IntegrationGuard):Promise<void>{
    await this.locked(task,async()=>{
      ...
      try{
        await guard([task.worktree,task.repository,op.candidate]);await inputs(task,op);
