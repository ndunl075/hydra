...
| 2. Managed delegation | Single-level child worktrees through the shared queue, parent suspension/wakeup, durable dispatch and stop/recovery. Cover two-slot capacity including the parent, simultaneous completions, restart during dispatch, budget holds, bounded retries/replanning, malformed proposals, cycles, provider failures, and no duplicate writers/launches. |
