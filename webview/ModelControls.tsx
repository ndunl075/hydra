...
export function ModelControls({ task, session, catalog, busy, send }: { task: Task; session?: SessionView; catalog?: ModelCatalog; busy: boolean; send: (message: ClientMessage) => void }) {
  ...
  return <details className="context-details"><summary>Model and effort <span>{task.modelSelection ? `${task.modelSelection.model} · ${task.modelSelection.effort}` : 'Official provider defaults'}</span></summary>
    {task.provider === 'claude' ? <p className="form-note">Claude's structured output can silently cap requested effort under organization policy. Hydra cannot verify effective effort here; choose settings in the official Claude client.</p> : <>
      <p className="form-note">These settings apply to managed Codex tasks. Discovery reads model metadata without creating a thread or submitting a prompt. The provider verifies access when you run the task.</p>
      ...
      <form className="model-controls" onSubmit={event => { event.preventDefault(); send({ type: 'saveModelSelection', id: task.id, selection }); }}>
        <label>Model<select disabled={busy || !editable || catalog?.status !== 'ready'} value={selection?.model || ''} onChange={event => { const model = models.find(model => model.model === event.target.value); setSelection(model ? { model: model.model, effort: model.efforts.includes(model.defaultEffort) ? model.defaultEffort : model.efforts[0] || '' } : null); }}><option value="">Official provider defaults</option>{selection && !selected && <option value={selection.model}>{selection.model} · refresh to verify</option>}{models.map(model => <option value={model.model} key={model.model}>{model.displayName} · {model.model}</option>)}</select></label>
        {selection && <label>Reasoning effort<select disabled={busy || !editable || !selected} value={selection.effort} onChange={event => setSelection({ ...selection, effort: event.target.value })}>{!selected?.efforts.includes(selection.effort) && <option value={selection.effort}>{selection.effort} · unavailable</option>}{selected?.efforts.map(effort => <option key={effort} value={effort}>{effort}</option>)}</select></label>}
        <div className="task-actions"><button type="submit" className="secondary" disabled={busy || !editable || !supported || !dirty}>Save model settings</button><button type="button" className="secondary" disabled={busy || !editable || !astra} onClick={() => setSelection({ model: astra!.model, effort: 'high' })}>Astra High</button>{selection && editable && <button type="button" className="text-button" disabled={busy} onClick={() => setSelection(null)}>Use provider defaults</button>}</div>
      </form>
      {!astra && <p className="form-note">Astra High is available only when this runtime advertises exactly gpt-6-astra with high effort. No substitute is selected.</p>}
      <p className="form-note">{editable ? 'Save settings before starting. Explicit selections lock when launched and are rechecked on resume.' : 'Settings are locked for this task. Follow-ups reapply its saved selection.'} No selection leaves provider configuration unchanged. A later provider reroute is reported; an explicitly configured run is stopped.</p>
    </>}
  </details>;
}
