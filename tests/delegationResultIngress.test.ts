...
const result = (key = 'child-1') => ({ version: 1, parentId, runId, childKey: key, dispatchKey: 'd'.repeat(24), baseCommit: base, commit, tree, changedPaths: ['src/example.ts'], decisions: 'Kept the result compact.', summary: 'Child completed its reviewed change.', unresolved: [], validations: [] as { command: string; status: 'passed'; evidenceRefs: string[] }[], evidence: [] as { id: string; kind: 'test'; label: string; path: string; sha256: string }[], dependencies: [] });
...
test('refuses caller validation and artifact claims that lack host-attested references', async () => {
  const f = await fixture(); try {
    const ingress = new DelegationResultIngress(f.journal), source = { child: child(), binding: binding(), dispatch: dispatch() };
    const claimedValidation = result();
    claimedValidation.validations = [{ command: 'npm.cmd test', status: 'passed', evidenceRefs: [] }];
    await assert.rejects(ingress.receive(claimedValidation, source), /host-attested references/);
    const claimedArtifact = result();
    claimedArtifact.evidence = [{ id: 'focused-log', kind: 'test', label: 'Focused test result', path: 'delegation-evidence/child/focused.log', sha256: 'f'.repeat(64) }];
    await assert.rejects(ingress.receive(claimedArtifact, source), /host-attested references/);
    assert.deepEqual((await f.journal.load(parentId, runId)).results, []);
  } finally { await rm(f.directory, { recursive: true, force: true }); }
});
