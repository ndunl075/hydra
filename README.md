...
- A persistent status-bar control switches modes. Editor mode keeps native files and diffs in the center, Explorer on the left, terminals below, and a compact agent conversation in the native secondary sidebar. **Hydra: Open Agent Conversation** reveals it at its current user-chosen location.
...
- Returning to Editor closes only the Agents tab and lets native tab history restore the preceding editor, including diffs and split groups. Mode switches preserve sidebar choices and terminal processes. The graph and orchestration controls stay in Agents.
- Editor and Agents share task selection, session output, approvals, and per-task follow-up drafts during the window session. New tasks can be created from the sidebar; **Start task** explicitly sends the brief. Follow-up text clears only after its queued request is accepted, and a late receipt cannot erase newer typing. Opening chat or switching modes never submits a provider request. The built-in module now requires Code OSS / VS Code 1.113 or newer for native secondary-sidebar contributions.
...
The CLI retains its existing authentication and environment. Hydra collects no credentials and bypasses no permissions. Explicit model/effort choices use only newly advertised settings and require an authoritative effective acknowledgement before the prompt; no selection leaves provider defaults unchanged. Main-session command/file approvals grant only the displayed request, with no persistent rule update. Unsupported interactive, MCP/network or subagent requests stop with official-client guidance. Provider rules/hooks still apply. See [Claude controls](docs/Claude_Controls.md) for candidate acceptance and scope. Existing API-key environment variables can affect the CLI's billing route; subscription billing is not asserted.
