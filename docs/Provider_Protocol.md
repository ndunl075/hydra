...
Hydra uses the unmodified official executable with `app-server --listen stdio://`. Version-specific TypeScript and JSON schemas were generated from pinned `@openai/codex@0.154.0`; the unmodified type dependency closure is checked into `src/core/generated/codex-0.154.0`. The stable initialization explicitly disables experimental APIs and attestation. No external transport/listener is opened, login is intermediated, or provider-private history is read. Model and effort follow official defaults unless an explicit per-task selection is saved; [verified model controls](Model_Controls.md) describe runtime discovery, effective-setting checks, and reroute handling.
