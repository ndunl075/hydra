...
Cancellation waits for the owned read process to finish terminating. Native acceptance also exposed transient Windows reader locks during task metadata replacement. Task, session and budget stores now retry only Windows sharing/permission errors for a bounded interval, keeping atomic replacement and preserving old records on persistent failure. Real file-lock tests cover recovery and retained records; no destination is deleted to force a write.
