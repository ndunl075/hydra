...
$report = [ordered]@{ schemaVersion = 1; status = 'started'; token = $evidence; packageRemoved = $false }
$package = $null
try {
  ...
  $report | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath ([string]$request.resultPath) -Encoding utf8
}
