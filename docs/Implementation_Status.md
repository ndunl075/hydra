...
| M3 structured sessions | Claude streaming/follow-up foundation implemented; milestone incomplete | CLI 2.1.270 streamed text, explicit session-ID follow-up, and usage confirmed with a real tools-disabled scratch test. Windows fixture/host tests cover persistence, recovery, malformed output, writer exclusion, and process-tree stop. Interactive approvals, graceful interruption, Codex App Server, and full interrupted-session acceptance remain pending |
