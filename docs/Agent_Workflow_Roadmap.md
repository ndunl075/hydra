...
The configurable managed-task limit defaults to two and is capped at eight per window. Terminal and managed-session launches now share a persistent queue, with reviewed prerequisite receipts and explicit reconciliation of uncertain writers after restart; see [task scheduling](Task_Scheduling.md). Reviewed integration, explicit briefs/local handoffs, aggregate reported usage and verified Codex model/effort controls have merged after Linux and native checks. The final combined installer runtime also passed Windows acceptance. Official-extension sessions are externally owned and are not counted by this limit. A worktree isolates checked-out files, not ports, databases, dependency services, or every filesystem access available to an agent. [Soft budgets](Soft_Budgets.md) warn or hold new local launches and turns using reported task/project history. A read-only Codex quota candidate exposes provider-reported account windows; live quota acceptance and verified Claude limits remain incomplete. Explicit resource/setup controls are locally tested candidates; feature CI, authenticated provider acceptance and measured efficiency remain incomplete. Reversible confirmed discard retains task checkouts for recovery; see [task discard](Task_Discard.md). See [implementation status](Implementation_Status.md).
...
Status: reviewed candidate integration, explicit acceptance checks, stale-state refusal and conflict recovery merged in PR #22. Reversible confirmed discard retires the task record after saved-state review and native confirmation, retaining its checkout and branch for explicit restore. Manual modal acceptance and permanent cleanup remain separate; later combined revisions require regression acceptance. See [Integration.md](Integration.md).
...
Status: task/project reported-usage accounting merged in PR #21; managed Codex catalog selection, pre-turn effective acknowledgement and resumed-setting validation merged in PR #23. Both passed native acceptance. Astra High is available only when the runtime advertises the exact model/effort; actual account access is unverified. Soft task/project budgets now warn or hold new work; see [Soft_Budgets.md](Soft_Budgets.md) for scope and acceptance gates. A passive provider usage-limit view and explicit pinned Codex quota refresh are implemented locally; see [Provider_Quotas.md](Provider_Quotas.md). Claude effective-effort and scoped approval controls are locally accepted candidates; full revision CI, authenticated Claude acceptance, verified Claude limits and live quota acceptance remain unfinished. See [Claude controls](Claude_Controls.md). See [Model_Controls.md](Model_Controls.md).
...
Status: Phase 1 host preparation exists. The first Phase 2 slice persistently reserves deterministic child dispatch/worktree identities before explicit Git creation and refuses automatic retry after uncertain creation. Automatic decomposition, Hydra child tasks, managed child execution, parent wakeup, and recovery remain unimplemented. See [Hydra helpers](Heads.md) (which replaced adaptive delegation) for the execution contract, phased delivery, and acceptance gates.

2026-09-18 architecture addition: after managed dispatch and recovery (Phase 2), deliver per-attempt verification evidence and blocking required checks (Phase 3), then a focused agent workspace and durable lifecycle indicators in the graph (Phase 4). Missing verification must never count as passing. Opening the workspace or evidence must not start processes or model turns. These planned additions follow the [review decision](Agentic_Workflow_Architecture.md#workflow-review-decision-2026-09-18) while preserving Hydra's native editor and efficiency policy; acceptance cases are defined in [the delegation specification](Adaptive_Delegation.md#verification-evidence-and-focused-agent-workspace).
...
The current release coordinates user-created tasks and explicit dependencies. Automatic task decomposition is now explicitly scoped in [Hydra helpers](Heads.md) (which replaced adaptive delegation), with separate implementation and release gates; unrestricted autonomous swarms remain out of scope. Parallel isolated agents are a core product goal; inexpensive, high-quality outcomes still require measurement.
