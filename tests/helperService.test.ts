  ...
  service = new HelperService({
    ...
    startRun: spec => {
      ...
      setTimeout(() => void options.script({
        ...
        commit: async (file, text) => { await mkdir(path.dirname(path.join(spec.worktree, file)), { recursive: true }); await writeFile(path.join(spec.worktree, file), text); await git(spec.worktree, ['add', '.']); await git(spec.worktree, ['commit', '-qm', `helper: ${file}`]); },
      }).catch(() => undefined), 0);
      return run;
    },
  });
...
test('a helper that commits in scope and reports done is checked and handed back to the lead', async () => {
  ...
  try {
    ...
    assert.equal(again.job_id, started.job_id); assert.equal(again.created, false, 'a repeated idempotency key never starts a second helper');
    const waited = await f.wait([started.job_id]);
    assert.equal(waited.all_settled, true);
    const [helper] = waited.helpers;
    ...
    assert.equal(f.runs.length, 1, 'one helper process');
...
test('failed checks re-prompt the helper, which fixes and passes; the third failure fails the job', async () => {
  const f = await fixture({ checks: passCheck, script: async helper => {
  ...
  try {
    ...
    const [helper] = (await f.wait([job_id])).helpers;
  ...
  try {
    ...
    const [helper] = (await never.wait([job_id])).helpers;
...
test('Hydra commits leftover changes; no changes and changes outside the write scope are refused', async () => {
  const f = await fixture({ script: async helper => {
    ...
    assert.match(await git(helper.spec.worktree, ['log', '-1', '--format=%s']), /Job scope \(Hydra helper [a-f0-9]{12}\)/);
...
test('a helper that stops without reporting is nudged once, then failed; one that exits is failed', async () => {
  const f = await fixture({ script: async helper => {
    helper.endTurn();
  ...
  try {
    ...
    const [helper] = (await crash.wait([job_id])).helpers;
    assert.equal(helper.state, 'failed'); assert.match(helper.reason, /exited \(code 3\)/);
  } finally { await crash.close(); }
});

test('the time limit stops a helper, and time spent waiting for an answer does not count', async () => {
  let clock = 0;
  const f = await fixture({ now: () => clock, script: async () => { /* works forever */ } });
  try {
    const { job_id } = await f.start('slow', { limits: { wall_clock_minutes: 1 } });
    await until(() => f.store.get(job_id)?.state === 'running' && f.runs.length === 1, 'helper process started');
    clock += 61_000;
    const [helper] = (await f.wait([job_id])).helpers;
...
test('a stuck helper waits for the lead, gets the answer as its tool result and continues', async () => {
  const f = await fixture({ script: async helper => {
  ...
  try {
    ...
    const blocked = (await f.wait([job_id])).helpers[0];
    assert.equal(blocked.state, 'blocked'); assert.equal(blocked.question, 'Use v1 or v2?');
    assert.match((await f.call('hydra_reply_to_helper', { job_id, message: '' })).error || '', /1–8000/);
    assert.deepEqual((await f.call('hydra_reply_to_helper', { job_id, message: 'v2' })).result, { job_id, delivered: true });
    const done = (await f.wait([job_id])).helpers[0];
    assert.equal(done.state, 'done'); assert.equal(done.summary, 'Used v2');
    assert.match((await f.call('hydra_reply_to_helper', { job_id, message: 'late' })).error || '', /not waiting for an answer/);
  } finally { await f.close(); }
});

test('cancel and Stop all end helpers; a dependency that fails fails its dependents; the queue respects the cap', async () => {
  ...
  try {
    ...
    await until(() => f.store.get(first.job_id)?.state === 'running', 'first helper running');
    assert.equal(f.store.get(third.job_id)?.state, 'queued', 'only one helper runs at a time here');
    ...
    assert.match((await f.call('hydra_get_helper', { job_id: 'ffffffffffff' })).error || '', /No helper/);
  } finally { await f.close(); }
});

test('the lead is warned when the helper cannot see uncommitted changes; checks come only from the lead folder', async () => {
...
test('scope matching, helper prompts, runner arguments and the supported CLI range', () => {
  ...
  assert.ok(!claude.join(' ').includes('WebFetch'), 'helpers get no web tools');
