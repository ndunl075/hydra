...
function ModelPicker({ selection, provider, catalogs, busy, send, onSelect }: {
  selection: ModelSelection | null; provider: Provider; catalogs: Partial<Record<Provider, ModelCatalog>>; busy: boolean; send: Send;
  ...
  return <details className="model-picker">
      ...
      {(['claude', 'codex'] as const).map(item => {
        ...
        return <div key={item} className="model-picker-group">
          <div className="model-picker-group-header">
            ...
            {catalog?.status !== 'checking' && <button type="button" className="text-button" disabled={busy} onClick={() => send({ type: 'checkModelsForProvider', provider: item })}>{catalog?.status === 'ready' ? 'Refresh' : catalog?.status === 'error' ? 'Retry' : 'Load models'}</button>}
          </div>
          {catalog?.status === 'checking' && <p className="form-note">Loading…</p>}
          {catalog?.status === 'error' && <p role="status" className="session-error">{catalog.error}</p>}
          {catalog?.status === 'ready' && !catalog.models.length && <p className="form-note">No models advertised.</p>}
          {catalog?.status === 'ready' && catalog.models.map(model => (
      ...
      <div className="task-prompt-box">
        ...
        <div className="task-prompt-toolbar">
          <div className="task-prompt-toolbar-group">
            ...
            <ModelPicker selection={selection} provider={draft.provider} catalogs={snapshot.draftModelCatalogs || {}} busy={snapshot.busy} send={send}
